PS1_ARW=''

function os_symbols() {
    if [ `uname` = Darwin ]; then
        echo ""
    elif [ "$(expr substr $(uname -s) 1 5)" = 'Linux' ]; then
        . /etc/os-release
        case $ID in
            ubuntu)
                echo ""
                ;;
            arch)
                echo ""
                ;;
            centos)
                echo ""
                ;;
            fedora)
                echo ""
                ;;
            debian)
                echo ""
                ;;
            *)
                echo ""
                ;;
        esac
    else
        echo '？'
    fi
}

function is_container() {
  [[ -f /.dockerenv ]] && return 0
  [[ -n "${APPTAINER_NAME:-}${SINGULARITY_NAME:-}" ]] && return 0
  grep -qaE '(docker|containerd|kubepods)' /proc/1/cgroup 2>/dev/null && return 0
  return 1
}

function show_venv() {
    [[ -n "${VIRTUAL_ENV:-}" ]] || return 0

    local name py ver
    name="$(basename "$(dirname "$VIRTUAL_ENV")")"

    py="$VIRTUAL_ENV/bin/python"
    [[ -x "$py" ]] || return 0
    ver="$("$py" -V 2>&1 | awk '{print $2}')"

    printf '(%s v%s)' "$name" "$ver"
}

function show_conda() {
    if [ -z "${CONDA_PROMPT_MODIFIER:-}" ]; then
        return
    fi

    printf '(conda:%s)' "${CONDA_PROMPT_MODIFIER}"
}

ESC_STR=$'\033'

PS1_SP3_R="${ESC_STR}[38;2;59;118;240;48;2;0;79;122m${PS1_ARW}"
PS1_SP4_R="${ESC_STR}[38;2;0;79;122;48;2;28;58;94m${PS1_ARW}"
SKIP_PY_R="${ESC_STR}[38;2;59;118;240;48;2;28;58;94m${PS1_ARW}"
PY_STY_R="${ESC_STR}[38;2;228;228;228;48;2;0;79;122m"

function show_python() {
    local conda venv

    conda="$(show_conda)"
    venv="$(show_venv)"

    if [[ -n "$conda" && -n "$venv" ]]; then
        printf '%s %s%s%s %s' "${PS1_SP3_R}" "${PY_STY_R}" "$conda" "$venv" "${PS1_SP4_R}"
    elif [ -n "$conda" ]; then
        printf '%s %s%s %s' "${PS1_SP3_R}" "${PY_STY_R}" "$conda" "${PS1_SP4_R}"
    elif [ -n "$venv" ]; then
        printf '%s %s%s %s' "${PS1_SP3_R}" "${PY_STY_R}" "$venv" "${PS1_SP4_R}"
    else
        printf '%s' "${SKIP_PY_R}"
    fi
}


__CMD_START_NS=""
__LAST_ELAPSED_MS=0
__HAVE_LAST=0
__IN_PROMPT=0
__CMD_ACTIVE=0
__FIRST_PROMPT=1

_now_ns() {
  if date +%s%N >/dev/null 2>&1; then
    date +%s%N
  else
    gdate +%s%N
  fi
}

timer_debug_trap() {
  (( __IN_PROMPT )) && return 0

  (( __CMD_ACTIVE )) && return 0

  local cmd="$BASH_COMMAND"

  case "$cmd" in
    timer_*|_now_ns*|format_duration* )
      return 0
      ;;
  esac

  __CMD_START_NS="$(_now_ns)"
  __CMD_ACTIVE=1
}

timer_prompt_hook() {
  __IN_PROMPT=1

  if (( __FIRST_PROMPT )); then
    __FIRST_PROMPT=0
    __HAVE_LAST=0
    __LAST_ELAPSED_MS=0
    __CMD_START_NS=""
    __CMD_ACTIVE=0
    __IN_PROMPT=0
    return 0
  fi

  if (( __CMD_ACTIVE )) && [[ -n "$__CMD_START_NS" ]]; then
    local end
    end="$(_now_ns)"
    __LAST_ELAPSED_MS=$(( (end - __CMD_START_NS) / 1000000 ))
    __HAVE_LAST=1
  else
    __LAST_ELAPSED_MS=0
    __HAVE_LAST=0
  fi

  __CMD_START_NS=""
  __CMD_ACTIVE=0
  __IN_PROMPT=0
}

format_duration() {
  local ms d h m s out=""
  ms=$1
  d=$(( ms / 86400000 )); ms=$(( ms % 86400000 ))
  h=$(( ms / 3600000 ));  ms=$(( ms % 3600000 ))
  m=$(( ms / 60000 ));    ms=$(( ms % 60000 ))
  s=$(( ms / 1000 ));     ms=$(( ms % 1000 ))

  (( d > 0 )) && out+="${d}d"
  (( h > 0 )) && out+=$(printf "%dh" "$h")
  (( m > 0 )) && out+=$(printf "%dm" "$m")
  (( s > 0 )) && out+=$(printf "%ds" "$s")
  out+=$(printf "%dms" "$ms")
  printf '%s' "$out"
}

timer_ps1() {
  local ms="${__LAST_ELAPSED_MS:-0}"
  if (( __HAVE_LAST )); then
      printf ' %s ' "$(format_duration "$ms")"
  else
      printf ' - '
  fi
}

trap 'timer_debug_trap' DEBUG

PROMPT_COMMAND="timer_prompt_hook${PROMPT_COMMAND:+; $PROMPT_COMMAND}"

OSSYM=`os_symbols`
export VIRTUAL_ENV_DISABLE_PROMPT=1

if is_container; then
    CONT='(container)'
else
    CONT=''
fi

# --- PS1 appearance ---
PS1_SP0="\[\e[38;2;102;150;255;48;2;173;199;255m\]${PS1_ARW}"
PS1_OS="\[\e[38;2;9;12;12;48;2;102;150;255m\] ${OSSYM} "

PS1_HST="\[\e[38;2;28;58;82;48;2;173;199;255m\] \h${CONT} "
PS1_SP1="\[\e[38;2;173;199;255;48;5;15m\]${PS1_ARW}"

PS1_USR="\[\e[38;2;28;58;82;48;5;15m\] \u "
PS1_SP2="\[\e[38;5;15;48;2;59;118;240m\]${PS1_ARW}"

PS1_DIR="\[\e[38;2;228;228;228;48;2;59;118;240m\] \w "

PS1_DUR="\[\e[38;2;141;251;210;48;2;28;58;94m\]"
PS1_SP5="\[\e[38;2;28;58;94;49m\]${PS1_ARW}"

PS1_BREAK="\n"
PS1_CMD="\[\e[0;36;49m\]\$ "
PS1_RST="\[\e[0;39;49m\]"

ITALIC_ENABLE="\[\e[3m\]"
ITALIC_DISABLE="\[\e[0m\]"

export PS1="$PS1_BREAK$PS1_OS$PS1_SP0$ITALIC_ENABLE$PS1_HST$PS1_SP1$PS1_USR$PS1_SP2$PS1_DIR\
\[\$(show_python)\]\
$PS1_DUR\$(timer_ps1)\
$PS1_SP5$ITALIC_DISABLE$PS1_BREAK$PS1_CMD$PS1_RST"
